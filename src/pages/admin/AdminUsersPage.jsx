...
export default function AdminUsersPage() {
  ...
  const saveUser = async (event) => {
    ...
    const payload = {
      ...
      email: form.email,
      role: form.role,
      assignment_area: form.assignment_area || null,
      address: form.address || null,
      whatsapp_phone: form.whatsapp_phone || null,
    };
    if (editing)
      payload.points_balance = Math.max(0, Number(form.points_balance || 0));
  ...
  const removeUser = async (user) => {
    if (!window.confirm(`Hapus akun ${user.name}? Data akun akan dihapus.`))
      return;
      ...
      >
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <h1 style={{ fontSize: "1.6rem", fontWeight: 800 }}>
            Manajemen User, Petugas, dan Admin
          </h1>
          <p className="text-faint">
            Kelola akses akun dan role operasional EcoPoints.
          </p>
        </div>
        {error && (
          <div
        ...
        >
          ...
          >
            ...
            {form.role === "petugas" && (
              <>
                <div className="form-group">
                  ...
                  <input
                    className="form-input"
                    value={form.assignment_area}
                    onChange={(e) => updateField("assignment_area", e.target.value)}
                    placeholder="Contoh: Drop Point Kecamatan A"
                  />
                </div>
                <div className="form-group">
                  <label className="form-label">Alamat Petugas</label>
                  <textarea
            ...
            {editing && (
              <div className="form-group">
                <label className="form-label">Saldo Poin</label>
                <input
                  className="form-input"
                  type="number"
                  min="0"
                  ...
                  onChange={(e) =>
                    updateField("points_balance", e.target.value)
                  }
                  required
                />
              </div>
            ...
            <table className="data-table">
              ...
              <tbody>
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setForm({
                              ...
                              password: "",
                              role: user.role,
                              points_balance: String(user.points_balance || 0),
                ...
                {!loading && !users.length && (
                  <tr>
                    <td colSpan="6" className="text-faint">
                      Belum ada akun.
                    </td>
                  </tr>
                )}
