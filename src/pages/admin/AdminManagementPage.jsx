...
const sectionMeta = {
  ...
  points: { title: 'Konfigurasi Poin', subtitle: 'Atur nilai rupiah dan poin untuk setiap jenis sampah.' },
  rewards: { title: 'Manajemen Katalog Hadiah', subtitle: 'Kelola hadiah yang tersedia untuk penukaran nasabah.' },
...
function buildReportFallback(deposits, transactions) {
  ...
  return {
    total_weight_kg: verified.reduce((total, item) => total + Number(item.weight_kg || 0), 0),
    total_users: 0,
    total_deposits: deposits.length,
    total_points_issued: transactions.filter(item => item.type === 'credit').reduce((total, item) => total + Number(item.amount || 0), 0),
    total_points_redeemed: transactions.filter(item => item.type === 'debit').reduce((total, item) => total + Number(item.amount || 0), 0),
...
function Stat({ label, value, suffix = '' }) {
  return <div className="card" style={{ padding: '1.25rem' }}><div className="text-faint font-mono" style={{ fontSize: '0.7rem', textTransform: 'uppercase' }}>{label}</div><div style={{ fontSize: '1.8rem', fontWeight: 800, marginTop: '0.5rem' }}>{value}{suffix && <small className="text-muted" style={{ fontSize: '0.8rem', marginLeft: '0.25rem' }}>{suffix}</small>}</div></div>;
}

function ErrorMessage({ message }) {
  return message ? <div style={{ padding: '0.75rem', marginBottom: '1rem', border: '1px solid #e5a39a', background: '#fff3f1', color: '#a63225' }}>{message}</div> : null;
}
...
export default function AdminManagementPage({ section = 'overview' }) {
  ...
  const load = useCallback(async () => {
    ...
    if (section === 'reports' || section === 'overview') result = await adminApi.getReportsSummary();
    if (result?.success) {
      if (section === 'reports' || section === 'overview') setReport(result.data || {});
      else setData(Array.isArray(result.data) ? result.data : []);
    } else if (result && (section === 'reports' || section === 'overview') && result.status === 404) {
      const [depositResult, transactionResult] = await Promise.all([depositApi.getAll(), pointApi.getTransactions()]);
      const deposits = depositResult.success && Array.isArray(depositResult.data) ? depositResult.data : [];
      const transactions = transactionResult.success && Array.isArray(transactionResult.data) ? transactionResult.data : [];
      setReport(buildReportFallback(deposits, transactions));
    } else if (result) setError(result.error || 'Data belum dapat dimuat. Pastikan API aktif.');
    setLoading(false);
  }, [section]);
  ...
  const removeItem = async (item) => {
    ...
    const result = section === 'points' ? await adminApi.deleteWasteType(item.id) : await adminApi.deleteReward(item.id);
    if (!result.success) setError(result.error || 'Data gagal dihapus.'); else load();
  };
  ...
  const updateRedemption = async (item, status) => {
    ...
    if (!result.success) setError(result.error || 'Status gagal diperbarui.'); else load();
  ...
  return <>
    ...
    <main className="container-wide" style={{ padding: '2rem 1rem' }}>
      ...
      {(section === 'overview' || section === 'reports') && <>
        {loading ? <div className="text-faint">Memuat laporan...</div> : <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
            <Stat label="Total Setoran" value={report?.total_deposits || 0} /><Stat label="Berat Terverifikasi" value={Number(report?.total_weight_kg || 0).toFixed(1)} suffix="kg" /><Stat label="Poin Diterbitkan" value={(report?.total_points_issued || 0).toLocaleString('id-ID')} /><Stat label="Poin Ditukar" value={(report?.total_points_redeemed || 0).toLocaleString('id-ID')} />
          </div>
          <div className="card" style={{ padding: '1.25rem' }}><h2 style={{ fontSize: '1rem', marginBottom: '1rem' }}>Distribusi berdasarkan jenis sampah</h2><div className="data-table-container"><table className="data-table"><thead><tr><th>Jenis</th><th>Berat</th><th>Setoran</th><th>Poin</th></tr></thead><tbody>{(report?.by_waste_type || []).map(item => <tr key={item.waste_type_id}><td>{item.waste_type_name}</td><td>{Number(item.total_weight_kg || 0).toFixed(1)} kg</td><td>{item.total_deposits}</td><td>{(item.total_points || 0).toLocaleString('id-ID')}</td></tr>)}{!(report?.by_waste_type || []).length && <tr><td colSpan="4" className="text-faint">Belum ada data laporan.</td></tr>}</tbody></table></div></div>
        </>}
      </>}

      {(section === 'points' || section === 'rewards') && <div style={{ display: 'grid', gridTemplateColumns: 'minmax(260px, 340px) 1fr', gap: '1.5rem', alignItems: 'start' }}>
      ...
      {section === 'redemptions' && <div className="data-table-container"><table className="data-table"><thead><tr><th>ID</th><th>Nasabah</th><th>Hadiah</th><th>Poin</th><th>Tanggal</th><th>Status</th><th>Aksi</th></tr></thead><tbody>{loading ? <tr><td colSpan="7">Memuat data...</td></tr> : data.map(item => <tr key={item.id}><td>#{item.id}</td><td>{item.user?.name || `User #${item.user_id}`}</td><td>{item.reward?.name || `Reward #${item.reward_id}`}</td><td>{item.points_used}</td><td>{formatDate(item.created_at)}</td><td><span className={`badge ${item.status === 'completed' ? 'badge-verified' : item.status === 'cancelled' ? 'badge-rejected' : 'badge-pending'}`}>{item.status}</span></td><td>{item.status === 'pending' && <><button className="btn btn-sm btn-primary" disabled={statusLoading === item.id} onClick={() => updateRedemption(item, 'completed')}>Selesaikan</button> <button className="btn btn-sm btn-danger" disabled={statusLoading === item.id} onClick={() => updateRedemption(item, 'cancelled')}>Batalkan</button></>}</td></tr>)}{!loading && !data.length && <tr><td colSpan="7" className="text-faint">Belum ada penukaran.</td></tr>}</tbody></table></div>}
    </main>
  </>;
}
